      ...
      <div style={{ position: "fixed", inset: 0, zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px", boxSizing: "border-box" }}>
        ...
        >
          ...
          {loading || !details ? (
            ...
            <>
              {/* ── 🌌 HERO BLEED BACKDROP (Scale & Visibility Fixed) ── */}
              ...
              {/* ── 🎬 MAIN CONTENT WRAPPER (Pushed down by 160px for cinematic reveal) ── */}
              <div style={{ position: "relative", zIndex: 10, padding: "160px 40px 40px 40px", display: "flex", flexDirection: "column" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "160px 1fr 200px", gap: "32px", alignItems: "flex-start" }}>
                  ...
                  {/* Left: Floating Poster & Trailer Button (Borderless & Sleeker) */}
                  <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                    <div style={{ borderRadius: "14px", overflow: "hidden", border: "1px solid rgba(255,255,255,0.1)", boxShadow: "0 25px 50px rgba(0,0,0,0.8)", backgroundColor: "#160B24" }}>
                       {details.poster_path ? (
                         <img src={getPosterUrl(details.poster_path)} alt={details.title || details.name} style={{ width: "100%", aspectRatio: "2/3", objectFit: "cover" }} />
                       ) : (
                  ...
                  <div style={{ display: "flex", flexDirection: "column", paddingTop: "10px" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "12px", fontSize: "11px", fontWeight: 700, color: "rgba(255,255,255,0.6)", marginBottom: "16px" }}>
                      ...
                      <span>|</span>
                      <span style={{ border: "1px solid rgba(255,255,255,0.2)", padding: "2px 6px", borderRadius: "6px", fontSize: "9px" }}>12+</span>
                    </div>

                    <p style={{ fontSize: "11.5px", color: "rgba(255,255,255,0.75)", lineHeight: 1.6, margin: "0 0 20px 0", maxWidth: "95%" }}>
                      {details.tagline && <span style={{ display: "block", marginBottom: "6px", fontWeight: 700, color: "#fff" }}>{details.tagline}</span>}
                      {details.overview}
                    </p>

                    <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", marginTop: "auto" }}>
                      {details.genres.map(g => (
                        <span key={g.id} style={{ backgroundColor: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.1)", padding: "4px 10px", borderRadius: "20px", fontSize: "9px", fontWeight: 700, color: "rgba(255,255,255,0.6)", textTransform: "uppercase", letterSpacing: "0.05em" }}>
                          {g.name}
                        </span>
                      ))}
                    </div>
                  ...
                  {/* Right: Glassmorphic Action Hub (Compact) */}
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", padding: "0 8px" }}>
                        <div onClick={() => handleInteractionToggle("watched")} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "6px", cursor: "pointer" }}>
                          <motion.div whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} style={{ color: isWatched ? "#4ade80" : "rgba(255,255,255,0.4)" }}>
                            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M12 4.5C7 4.5 2.73 8.11 1 12c1.73 3.89 6 7.5 11 7.5s9.27-3.61 11-7.5c-1.73-3.89-6-7.5-11-7.5zM12 17c-2.76 0-5-2.24-5-5s2.24-5 5-5 5 2.24 5 5-2.24 5-5 5zm0-8c-1.66 0-3 1.34-3 3s1.34 3 3 3 3-1.34 3-3-1.34-3-3-3z"/></svg>
                          </motion.div>
                          <span style={{ fontSize: "8px", fontWeight: 700, color: "rgba(255,255,255,0.5)" }}>Watched</span>
                        </div>
                        <div onClick={() => handleInteractionToggle("liked")} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "6px", cursor: "pointer" }}>
                          ...
                          <span style={{ fontSize: "8px", fontWeight: 700, color: "rgba(255,255,255,0.5)" }}>Like</span>
                        </div>
                ...
                <div style={{ marginTop: "32px", paddingTop: "24px", borderTop: "1px solid rgba(255,255,255,0.05)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "20px", fontWeight: 900 }}>
                      <span>★</span> {(details.vote_average / 2).toFixed(1)}
                    </div>
                  </div>
                  <div style={{ display: "flex", gap: "12px" }}>
                    <span style={{ padding: "4px 10px", borderRadius: "20px", border: "1px solid rgba(255,255,255,0.1)", fontSize: "10px", fontWeight: 700, display: "flex", alignItems: "center", gap: "6px" }}>
                      <b style={{ color: "#fff" }}>IMDb</b> {details.vote_average.toFixed(1)}
                    </span>
                    <span style={{ padding: "4px 10px", borderRadius: "20px", border: "1px solid rgba(255,255,255,0.1)", fontSize: "10px", fontWeight: 700, display: "flex", alignItems: "center", gap: "6px" }}>
                      <b style={{ color: "#ef4444" }}>🍅</b> {Math.round((details.vote_average / 10) * 100)}%
                    </span>
                    <span style={{ padding: "4px 10px", borderRadius: "20px", border: "1px solid rgba(255,255,255,0.1)", fontSize: "10px", fontWeight: 700, display: "flex", alignItems: "center", gap: "6px", color: "rgba(255,255,255,0.6)" }}>
                ...
                {/* ── 👥 CAST & CREW (Hard Fix: Bulletproof Avatar Fallbacks) ── */}
                {cast.length > 0 && (
                  <div style={{ marginTop: "24px", paddingTop: "24px", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
                    ...
                    <div className="no-scrollbar" style={{ display: "flex", gap: "20px", overflowX: "auto", paddingBottom: "12px" }}>
                      {cast.slice(0, 15).map(actor => (
                        <div key={actor.id} style={{ display: "flex", flexDirection: "column", alignItems: "center", width: "56px", flexShrink: 0, gap: "8px", cursor: "pointer" }}>
                          <div style={{ width: "56px", height: "56px", borderRadius: "50%", overflow: "hidden", border: "1px solid rgba(255,255,255,0.1)", backgroundColor: "#160B24" }}>
                            {actor.profile_path ? (
                              <img 
                            ...
                            <div className={actor.profile_path ? 'hidden' : ''} style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", color: "rgba(255,255,255,0.4)", fontSize: "16px", fontWeight: 800, background: "linear-gradient(135deg, rgba(255,255,255,0.05) 0%, transparent 100%)" }}>
                              {actor.name.charAt(0)}
                            </div>
                          </div>
                          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", lineHeight: 1.2 }}>
                            <span style={{ fontSize: "9px", fontWeight: 800, color: "#fff", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", width: "100%" }}>{actor.name}</span>
                            <span style={{ fontSize: "8px", color: "rgba(255,255,255,0.4)", marginTop: "2px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", width: "100%" }}>{actor.character}</span>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                {/* ── 🎞️ SIMILAR FILMS (Hard Fix: Premium Title Fallbacks) ── */}
                {similar.length > 0 && (
                  <div style={{ marginTop: "24px", paddingTop: "24px", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
                      <h3 style={{ margin: 0, fontSize: "11px", fontWeight: 800, color: "#fff", letterSpacing: "0.02em", textTransform: "uppercase" }}>Similar {mediaType === "tv" ? "Shows" : "Films"} <span style={{ color: "rgba(255,255,255,0.3)", marginLeft: "4px" }}>{'>'}</span></h3>
                    </div>
                    <div className="no-scrollbar" style={{ display: "flex", gap: "12px", overflowX: "auto", paddingBottom: "12px" }}>
                      {similar.slice(0, 10).map((item) => (
                        <motion.div
                          key={item.id}
                          whileHover={{ scale: 1.05, y: -4 }}
                          whileTap={{ scale: 0.95 }}
                          style={{ width: "110px", flexShrink: 0, cursor: "pointer", display: "flex", flexDirection: "column", gap: "8px" }}
                        >
                          <div style={{ width: "100%", aspectRatio: "2/3", borderRadius: "10px", overflow: "hidden", border: "1px solid rgba(255,255,255,0.05)", backgroundColor: "#160B24", boxShadow: "0 10px 20px rgba(0,0,0,0.5)" }}>
                             {item.poster_path ? (
                               <img 
                             ...
                             <div className={item.poster_path ? 'hidden' : ''} style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", padding: "12px", textAlign: "center", fontSize: "10px", fontWeight: 800, color: "rgba(255,255,255,0.4)", background: "linear-gradient(135deg, rgba(255,255,255,0.05) 0%, transparent 100%)" }}>
                ...
                <div style={{ marginTop: "24px", paddingTop: "24px", borderTop: "1px solid rgba(255,255,255,0.05)", paddingBottom: "16px" }}>
                  ...
                  <div style={{ backgroundColor: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.04)", borderRadius: "16px", padding: "20px" }}>
                    <h4 style={{ margin: "0 0 4px 0", fontSize: "12px", fontWeight: 800, color: "#fff" }}>{details.title || details.name}</h4>
                    <span style={{ fontSize: "8px", fontWeight: 700, color: "rgba(255,255,255,0.4)", textTransform: "uppercase", letterSpacing: "0.05em" }}>
                      {details.genres[0]?.name || "Media"} Epic
                    </span>
                    <p style={{ margin: "12px 0 0 0", fontSize: "11.5px", color: "rgba(255,255,255,0.6)", lineHeight: 1.6 }}>
                      {details.overview}
                    </p>
                  </div>
                </div>
