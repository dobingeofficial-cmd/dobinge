      ...
      <div style={{ position: "fixed", inset: 0, zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px", boxSizing: "border-box" }}>
        ...
        >
          ...
          {loading || !details ? (
            ...
            <>
              {/* ── 🌌 HERO BLEED BACKDROP ── */}
              <div style={{ position: "absolute", top: 0, left: 0, width: "100%", height: "550px", zIndex: 0, pointerEvents: "none" }}>
                {details.backdrop_path && (
                  <img src={getBackdropUrl(details.backdrop_path)} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", opacity: 0.5, WebkitMaskImage: "linear-gradient(to bottom, black 30%, transparent 100%)", maskImage: "linear-gradient(to bottom, black 30%, transparent 100%)" }} />
                )}
              ...
              <div style={{ position: "relative", zIndex: 10, padding: "56px 48px 48px 48px", display: "flex", flexDirection: "column" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "220px 1fr 280px", gap: "48px", alignItems: "flex-start" }}>
                  
                  {/* Left: Floating Poster & Trailer Button */}
                  <div style={{ display: "flex", flexDirection: "column", gap: "16px", marginTop: "20px" }}>
                    <div style={{ padding: "8px", backgroundColor: "#EAE5D9", borderRadius: "16px", boxShadow: "0 25px 50px rgba(0,0,0,0.8)" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", paddingTop: "40px" }}>
                    <h1 style={{ fontSize: "3.8rem", fontWeight: 900, letterSpacing: "-0.03em", lineHeight: 1.1, margin: "0 0 12px 0", textShadow: "0 4px 20px rgba(0,0,0,0.8)" }}>{details.title || details.name}</h1>
                    ...
                    <p style={{ fontSize: "14px", color: "rgba(255,255,255,0.75)", lineHeight: 1.6, margin: "0 0 32px 0", maxWidth: "95%" }}>
                      {details.tagline && <span style={{ display: "block", marginBottom: "12px", fontWeight: 600, color: "#fff" }}>{details.tagline}</span>}
                      {details.overview}
                    </p>

                    <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", marginTop: "auto" }}>
                      {details.genres.map(g => (
                        <span key={g.id} style={{ backgroundColor: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.1)", padding: "6px 14px", borderRadius: "20px", fontSize: "11px", fontWeight: 700, color: "rgba(255,255,255,0.6)" }}>
                          {g.name}
                        </span>
                      ))}
                    </div>
                  ...
                  <div style={{ paddingTop: "40px" }}>
                    <div style={{ backgroundColor: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.04)", borderRadius: "24px", padding: "24px", display: "flex", flexDirection: "column", gap: "24px", backdropFilter: "blur(20px)", boxShadow: "0 20px 40px rgba(0,0,0,0.5)" }}>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                        ...
                        <button style={{ padding: "10px 0", borderRadius: "30px", backgroundColor: "rgba(255,255,255,0.03)", color: "#fff", border: "1px solid rgba(255,255,255,0.05)", fontSize: "12px", fontWeight: 700, cursor: "pointer" }}>Review</button>
                      </div>
                ...
                <div style={{ marginTop: "48px", paddingTop: "32px", borderTop: "1px solid rgba(255,255,255,0.05)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                    <h3 style={{ margin: 0, fontSize: "14px", fontWeight: 800, color: "#fff", letterSpacing: "0.02em" }}>Rating & Reviews</h3>
                ...
                {cast.length > 0 && (
                  <div style={{ marginTop: "40px", paddingTop: "32px", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
                    <h3 style={{ margin: "0 0 24px 0", fontSize: "15px", fontWeight: 800, color: "#fff", letterSpacing: "0.02em" }}>Cast & Crew</h3>
                    <div className="no-scrollbar" style={{ display: "flex", gap: "32px", overflowX: "auto", paddingBottom: "16px" }}>
                      {cast.slice(0, 15).map(actor => (
                        <div key={actor.id} style={{ display: "flex", flexDirection: "column", alignItems: "center", width: "80px", flexShrink: 0, gap: "10px", cursor: "pointer" }}>
                          <div style={{ width: "72px", height: "72px", borderRadius: "50%", overflow: "hidden", border: "1px solid rgba(255,255,255,0.1)", backgroundColor: "#0b0612" }}>
                            {actor.profile_path ? (
                          ...
                          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", lineHeight: 1.2 }}>
                            ...
                            <span style={{ fontSize: "10px", color: "rgba(255,255,255,0.4)", marginTop: "4px" }}>{actor.character}</span>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                {similar.length > 0 && (
                  <div style={{ marginTop: "40px", paddingTop: "32px", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "24px" }}>
                      <h3 style={{ margin: 0, fontSize: "15px", fontWeight: 800, color: "#fff", letterSpacing: "0.02em" }}>Similar {mediaType === "tv" ? "Shows" : "Films"} <span style={{ color: "rgba(255,255,255,0.3)", marginLeft: "4px" }}>{'>'}</span></h3>
                    </div>
                    <div className="no-scrollbar" style={{ display: "flex", gap: "16px", overflowX: "auto", paddingBottom: "16px" }}>
                      {similar.slice(0, 10).map((item) => (
                        <motion.div
                          ...
                          whileTap={{ scale: 0.95 }}
                          style={{ width: "140px", flexShrink: 0, cursor: "pointer", display: "flex", flexDirection: "column", gap: "8px" }}
                        >
                          <div style={{ width: "100%", aspectRatio: "2/3", borderRadius: "12px", overflow: "hidden", border: "1px solid rgba(255,255,255,0.05)", backgroundColor: "rgba(255,255,255,0.02)", boxShadow: "0 10px 20px rgba(0,0,0,0.5)" }}>
                             {item.poster_path ? (
                               ...
                               <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", padding: "12px", textAlign: "center", fontSize: "12px", fontWeight: 700, color: "rgba(255,255,255,0.3)" }}>
                ...
                {/* ── 📖 STORYLINE (BOTTOM) ── */}
                <div style={{ marginTop: "40px", paddingTop: "32px", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
                  <h3 style={{ margin: "0 0 16px 0", fontSize: "15px", fontWeight: 800, color: "#fff", letterSpacing: "0.02em" }}>Storyline</h3>
                  <div style={{ backgroundColor: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.04)", borderRadius: "16px", padding: "24px" }}>
                    <h4 style={{ margin: "0 0 4px 0", fontSize: "14px", fontWeight: 800, color: "#fff" }}>{details.title || details.name}</h4>
                    ...
                    <p style={{ margin: "16px 0 0 0", fontSize: "13px", color: "rgba(255,255,255,0.6)", lineHeight: 1.6 }}>
                      {details.overview}
                    </p>
                  </div>
                </div>
