...
export default function SavedView({ onSelectMedia }: { onSelectMedia?: (media: any) => void }) {
  ...
  // 🎯 HARD FIX: Define Edge Gateway URL
  ...
  const healDataPipeline = async (rawData: any[]) => {
    const healed = await Promise.all(rawData.map(async (d: any) => {
      ...
      let realTab = d.interaction_type || d.interactionType || "watchlist";

      // 🎯 HARD FIX: Route Auto-Healing via Cloudflare Edge
      if (!recoveredMedia && mediaId && proxyUrl) {
        try {
          if (d.media_type === "tv") {
            let resTv = await fetch(`${proxyUrl}/api/tv/${mediaId}`);
            if (resTv.ok) recoveredMedia = { ...(await resTv.json()), media_type: "tv" };
          } else if (d.media_type === "movie") {
            let resMovie = await fetch(`${proxyUrl}/api/movie/${mediaId}`);
            if (resMovie.ok) recoveredMedia = { ...(await resMovie.json()), media_type: "movie" };
          } else {
            // Legacy collision resolution
            let resMovie = await fetch(`${proxyUrl}/api/movie/${mediaId}`);
            let resTv = await fetch(`${proxyUrl}/api/tv/${mediaId}`);
            
            let movieData = resMovie.ok ? await resMovie.json() : null;
            let tvData = resTv.ok ? await resTv.json() : null;

            if (movieData && tvData) {
              recoveredMedia = (tvData.popularity || 0) > (movieData.popularity || 0) 
                ? { ...tvData, media_type: "tv" } 
                : { ...movieData, media_type: "movie" };
            } else if (tvData) {
              recoveredMedia = { ...tvData, media_type: "tv" };
            } else if (movieData) {
              recoveredMedia = { ...movieData, media_type: "movie" };
            }
          }
        } catch (e) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      setIsSearching(true);
      try {
        // 🎯 HARD FIX: Route Live Search via Cloudflare Edge
        if (!proxyUrl) return;
        const res = await fetch(`${proxyUrl}/api/search/multi?query=${encodeURIComponent(searchQuery)}`);
  ...
  const handleAddNewItem = async (media: any) => {
    ...
    const isDuplicate = savedItems.some(item => item.media_id === mediaToSave.id && item.interaction_type === activeTab);
    ...
    setSavedItems(prev => [newItem, ...prev.filter(item => !(item.media_id === mediaToSave.id && item.interaction_type === activeTab))]);
    ...
    if (user) {
      ...
      await supabase.from("interactions")
        .delete()
        .eq("user_id", user.id)
        .eq("media_id", mediaToSave.id)
        .eq("interaction_type", activeTab);

      let finalError = null;

      // ATTEMPT 1: Target media_data column (JSONB)
      let { error: e1 } = await supabase.from("interactions").insert({
        user_id: user.id, media_id: mediaToSave.id, interaction_type: activeTab, media_type: mediaToSave.media_type, media_data: cleanMediaData 
      });

      if (e1) {
        // ATTEMPT 2: Target media_data column (TEXT stringified)
        if (e1.message.includes("type") || e1.message.includes("syntax")) {
          const { error: e1s } = await supabase.from("interactions").insert({
            user_id: user.id, media_id: mediaToSave.id, interaction_type: activeTab, media_type: mediaToSave.media_type, media_data: JSON.stringify(cleanMediaData)
          });
          e1 = e1s;
        }
      }
      ...
      if (e1 && e1.code === 'PGRST204') {
        // ATTEMPT 3: Target mediaData column (CamelCase JSONB)
        let { error: e2 } = await supabase.from("interactions").insert({
          user_id: user.id, media_id: mediaToSave.id, interaction_type: activeTab, media_type: mediaToSave.media_type, mediaData: cleanMediaData
        });

        if (e2) {
           // ATTEMPT 4: Target mediaData column (TEXT stringified)
           if (e2.message.includes("type") || e2.message.includes("syntax")) {
             const { error: e2s } = await supabase.from("interactions").insert({
               user_id: user.id, media_id: mediaToSave.id, interaction_type: activeTab, media_type: mediaToSave.media_type, mediaData: JSON.stringify(cleanMediaData)
             });
             e2 = e2s;
           }
        }
        ...
        if (e2 && e2.code === 'PGRST204') {
          // ATTEMPT 5: God-Mode Fallback (Save IDs and media_type only, skip metadata entirely)
          const { error: e3 } = await supabase.from("interactions").insert({
            user_id: user.id, media_id: mediaToSave.id, interaction_type: activeTab, media_type: mediaToSave.media_type
          });
          finalError = e3;
        } else {
          finalError = e2;
      ...
      if (finalError) {
        alert(`Supabase Rejected Save: ${finalError.message}`);
        // Rollback Optimistic UI immediately on total failure
        ...
        <div style={{ display: "flex", gap: "12px" }}>
          ...
          <motion.button
            ...
            style={{
              padding: "10px 20px",
              borderRadius: "24px",
              backgroundColor: isEditMode ? "rgba(239, 68, 68, 0.15)" : "rgba(255,255,255,0.04)",
              border: isEditMode ? "1px dashed rgba(239, 68, 68, 0.4)" : "1px dashed rgba(255,255,255,0.3)",
              color: isEditMode ? "#ef4444" : "rgba(255,255,255,0.6)",
              fontSize: "10px",
              fontWeight: 800,
              textTransform: "uppercase",
              letterSpacing: "0.1em",
          ...
          {categorizedData.map((category) => {
            ...
            return (
              <div key={category.id} style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
                {/* Section Header (Interactive Dropdown) */}
                <div 
                ...
                >
                  ...
                  {needsExpand && (
                    <motion.div
                      ...
                      style={{ width: "28px", height: "28px", display: "flex", alignItems: "center", justifyContent: "center", borderRadius: "50%", backgroundColor: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", backdropFilter: "blur(10px)" }}
                    >
                      ...
                      >
                        {item.media_data ? (
                          <PremiumMediaCard 
                            ...
                            onClick={() => {
                              if (!isEditMode && onSelectMedia) {
                                onSelectMedia({ ...item.media_data, mediaType: item.media_data.media_type || "movie" });
                              }
                            }} 
                          />
                        ) : null}
                        ...
                        {isEditMode && (
                          <motion.div
                            ...
                            onClick={(e: React.MouseEvent) => handleRemoveItem(item.media_id, e)}
                            style={{
                              position: "absolute", top: "-8px", right: "-8px", width: "24px", height: "24px",
                              backgroundColor: "#ef4444", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center",
                              cursor: "pointer", zIndex: 20, boxShadow: "0 4px 10px rgba(239, 68, 68, 0.5)", border: "2px solid #0a0612"
                            }}
        ...
        {isAddModalOpen && (
          <div style={{ position: "fixed", inset: 0, zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center", padding: "16px" }}>
            ...
            <motion.div
              ...
              style={{ position: "relative", width: "100%", maxWidth: "600px", height: "80vh", maxHeight: "700px", backgroundColor: "rgba(10, 5, 15, 0.95)", border: "1px solid rgba(168, 85, 247, 0.3)", borderRadius: "32px", display: "flex", flexDirection: "column", overflow: "hidden", boxShadow: "0 30px 60px rgba(0,0,0,0.8)" }}
            >
              <div style={{ padding: "24px 24px 16px", borderBottom: "1px solid rgba(255,255,255,0.05)", display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0 }}>
                <div>
                  ...
                  <p style={{ margin: "4px 0 0 0", fontSize: "10px", color: "rgba(168,85,247,0.8)", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em" }}>
                    Query Cinematic Database
                  </p>
                </div>
              ...
              <div style={{ padding: "24px", flexShrink: 0 }}>
                 <div style={{ position: "relative" }}>
                   <input
                     ...
                     style={{ width: "100%", padding: "16px 20px 16px 52px", borderRadius: "20px", backgroundColor: "rgba(255,255,255,0.04)", border: "1px solid rgba(192, 132, 252, 0.4)", color: "#fff", fontSize: "14px", outline: "none", boxSizing: "border-box", boxShadow: "inset 0 0 20px rgba(168, 85, 247, 0.05)", transition: "all 0.3s" }}
                     onFocus={e => e.target.style.backgroundColor = "rgba(255,255,255,0.08)"}
                 ...
                 {isSearching ? (
                   ...
                   <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(110px, 1fr))", gap: "16px" }}>
                     {searchResults.map((res) => {
                       const isAlreadyAdded = savedItems.some(item => item.media_id === res.id && item.interaction_type === activeTab);
                       return (
                         <motion.div 
                           key={res.id} 
                           whileHover={{ scale: 1.05 }} 
                   ...
                   <div style={{ display: "flex", justifyContent: "center", alignItems: "center", height: "100%", color: "rgba(255,255,255,0.4)", fontSize: "12px", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.1em" }}>
                     No signals found in neural core
                   </div>
                 ) : (
