        ...
        rawData.map(async (d: any) => {
          ...
          if ((!recoveredMedia || !recoveredMedia.poster_path) && mediaId) {
            try {
              ...
              let res;
              if (proxyUrl) {
                res = await fetch(`${proxyUrl}/api/${endpoint}/${mediaId}?language=en-US`);
              } else if (tmdbKey) {
                      ...
                      >
                          ...
                          <PremiumMediaCard
                            ...
                            onClick={() => {
                              ...
                              if (!isEditMode) {
                                setSelectedMedia({
                                  ...item.media_data,
                                  mediaType: item.media_data.media_type || "movie",
                                });
                              }
                            }}
                          />
                        ...
                        {isEditMode && (
                          <motion.div
                            ...
                            whileTap={{ scale: 0.9 }}
                          >
                            <svg width="12" height="12" fill="none" stroke="#fff" strokeWidth="3" strokeLinecap="round">
                ...
                {isSearching ? (
                  ...
                  >
                    {searchResults.map((res) => {
                      const isAlreadyAdded = savedItems.some(
                        (item) => String(item.media_id) === String(res.id) && item.interaction_type === activeTab
                      );
                      return (
                        <motion.div
                          key={res.id}
